...
class MobileNetV3Backbone(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.return_nodes = {
            'features.3': 'low_level_feat',
            'features.6': 'mid_level_feat',
            'features.9': 'high_level_feat',
            'features.16': 'final_feat'
        }
    ...
    def forward(self, x):
        ...
        return (
            features['low_level_feat'],
            features['mid_level_feat'],
            features['high_level_feat'],
            features['final_feat']
        )
...
class DeepLab(nn.Module):
    def __init__(self, num_classes, backbone="mobilenet", pretrained=True, downsample_factor=16):
        ...
        self.dense_aspp_conv = nn.Sequential(
            nn.Conv2d(2240, 256, 1),  # 修改1344为2240
            nn.BatchNorm2d(256),
    ...
    def forward(self, x):
        ...
        x = self.dense_aspp_conv(x)  # [B, 256, H/16, W/16]
        ...
        cff_out = self.cff(low_feat, mid_up)  # [B, 256, H/4, W/4]


        x_up = F.interpolate(x, size=(h_low, w_low),
                             mode='bilinear', align_corners=True)


        x = self.fusion_conv(x_up + cff_out)
