...
class _DenseASPPBlock(nn.Module):
    def __init__(self, in_channels, inter_channels1, inter_channels2, norm_layer=nn.BatchNorm2d):
        super().__init__()
        self.aspp_3 = _DenseASPPConv(in_channels, inter_channels1, inter_channels2, 3)
        self.aspp_6 = _DenseASPPConv(in_channels + inter_channels2, inter_channels1, inter_channels2, 6)
        self.aspp_12 = _DenseASPPConv(in_channels + inter_channels2 * 2, inter_channels1, inter_channels2, 12)
        self.aspp_18 = _DenseASPPConv(in_channels + inter_channels2 * 3, inter_channels1, inter_channels2, 18)
        self.aspp_24 = _DenseASPPConv(in_channels + inter_channels2 * 4, inter_channels1, inter_channels2, 24)
...
class StripPooling(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w = x.size()
        x1 = self.conv1(x)
        x2 = F.interpolate(self.conv2(F.adaptive_avg_pool2d(x1, (1, w))), (h, w), ** self._up_kwargs)
        x3 = F.interpolate(self.conv3(F.adaptive_avg_pool2d(x1, (h, 1))), (h, w), ** self._up_kwargs)
        return x + F.relu(self.conv4(F.relu(x2 + x3)))
...
class ASPP(nn.Module):
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
        ...
        self.branch1 = nn.Sequential(
            nn.Conv2d(dim_in, dim_out, 1, padding=0, dilation=rate),
...
class DeepLab(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.use_ca = use_ca
        self.use_dense_aspp = use_dense_aspp
        ...
        self.ca = CoordAtt(in_channels, in_channels) if use_ca else nn.Identity()

        if use_dense_aspp:
            self.aspp = _DenseASPPBlock(in_channels=512, inter_channels1=64, inter_channels2=64)
            self.dense_conv = nn.Sequential(
                nn.Conv2d(1344, 256, 1),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True)
            )
        else:
            self.aspp = ASPP(dim_in=in_channels, dim_out=256)
            self.dense_conv = nn.Identity()
        ...
        if use_cff:
            ...
            self.shortcut_conv = nn.Sequential(
                nn.Conv2d(self.low_level_channels, 48, 1),
                nn.BatchNorm2d(48),
                nn.ReLU(inplace=True)
            )
    ...
    def forward(self, x):
        ...
        final_feat = self.ca(final_feat) if self.use_ca else final_feat

        aspp_feat = self.aspp(final_feat)
        aspp_feat = self.dense_conv(aspp_feat)

        if self.use_cff:
            target_size = (final_feat.shape[2] * 4, final_feat.shape[3] * 4)

            low_up = F.interpolate(low_feat, size=target_size, mode='bilinear', align_corners=True)
            mid_up = F.interpolate(mid_feat, size=target_size, mode='bilinear', align_corners=True)

            cff_out = self.cff(low_up, mid_up)

            aspp_up = F.interpolate(aspp_feat, size=target_size, mode='bilinear', align_corners=True)
            x = cff_out + aspp_up
        else:
            low_feat = self.shortcut_conv(low_feat)
            aspp_feat = F.interpolate(aspp_feat, size=low_feat.shape[2:], mode='bilinear', align_corners=True)
            x = torch.cat([low_feat, aspp_feat], dim=1)
...
if __name__ == "__main__":
    ...
    model = DeepLab(num_classes=7, use_ca=False, use_dense_aspp=False, use_cff=False)

    model = DeepLab(num_classes=7, use_ca=True, use_dense_aspp=False, use_cff=False)

    model = DeepLab(num_classes=7, use_ca=False, use_dense_aspp=True, use_cff=False)

    model = DeepLab(num_classes=7, use_ca=False, use_dense_aspp=False, use_cff=True)
